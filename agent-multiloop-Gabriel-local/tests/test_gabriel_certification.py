...
class TestSpectralRatio:
    ...
    def test_ratio_symmetric_nxn(self):
        ...
        # A = (3, 5, 7) ; B = (11, 13, 17)
        # RsP = sum(SA(A)) / sum(SB(B)) ~= 1/2
        A = [2, 3, 4]  # positions des premiers 3, 5, 7
        B = [5, 6, 7]  # positions des premiers 11, 13, 17
        
        sum_sa_a = sum((3.25/2) * (2**n) - 2 for n in A)
        sum_sb_b = sum((6.5/2) * (2**n) - 66 for n in B)
        
        ratio = sum_sa_a / sum_sb_b
        assert abs(ratio - 0.5) < 0.1, f"Ratio nxn should be ~0.5, got {ratio}"
        print(f"✓ Ratio nxn: {ratio:.4f} ≈ 0.5")
    
    def test_ratio_asymmetric_chaotic(self):
        """Cas asymétrique chaotique: blocs de tailles différentes, ordre quelconque"""
        ...
        ratio = sum_sa_a / sum_sb_b
        # Asymétrique chaotique: ratio ≈ 0.5 (avec petit reste)
        assert abs(ratio - 0.5) < 0.2, f"Ratio chaotic should be ~0.5±0.2, got {ratio}"
        print(f"✓ Ratio asymétrique chaotique: {ratio:.4f} ≈ 0.5")
...
class TestPrimeReconstruction:
    ...
    def test_sa_sb_digamma_formulas(self):
        """Vérifier les formules spectrales: SA, SB, digamma"""
        n = 17
        p = 59
        ...
        digamma = sb - 64 * p
        assert digamma == 425854.0, f"digamma({n},{p}) should be 425854.0, got {digamma}"
...
class TestGapCalculation:
    ...
    def test_gap_negative_negative_19_5(self, gap_solver):
        """Cas (-,-): écart entre -19 et -5 (attendu: -13)"""
        ...
        assert result.p1 == -19
        assert result.p2 == -5
        assert result.gap_count == 13, f"Gap (-19,-5) should be 13, got {result.gap_count}"
        print(f"✓ Gap (-,-): between -19 and -5 = {result.gap_count} numbers")
    
    def test_gap_negative_negative_41_5(self, gap_solver):
        """Cas (-,-): écart entre -41 et -5 (attendu: -35)"""
        result = gap_solver.solve_gap(-41, -5)
        assert result is not None, "Gap solver should return result"
        assert result.gap_type == "negative_negative"
        assert result.gap_count == 35, f"Gap (-41,-5) should be 35, got {result.gap_count}"
        print(f"✓ Gap (-,-): between -41 and -5 = {result.gap_count} numbers")
    
    def test_gap_negative_negative_3_47(self, gap_solver):
        """Cas (-,-): écart entre -3 et -47 (attendu: -50)"""
        result = gap_solver.solve_gap(-3, -47)
        assert result is not None, "Gap solver should return result"
        assert result.gap_type == "negative_negative"
        assert result.gap_count == 50, f"Gap (-3,-47) should be 50, got {result.gap_count}"
        print(f"✓ Gap (-,-): between -3 and -47 = {result.gap_count} numbers")
...
class TestHOLScriptGeneration:
    ...
    def test_hol_script_correct_digamma(self, adapter):
        ...
        # Vérifier que le script contient la BONNE valeur
        assert "425854.0" in script, "Script should contain digamma = SB - 64*p = 425854.0"
        
        # Vérifier qu'il ne contient PAS juste 'p'
        # (Le bug était: digamma_calc 17 59 = 59.0)
        assert "59.0" not in script or "425854" in script
        
        print(f"✓ HOL script: digamma = 425854.0 (SB - 64*p), NOT 59.0")
