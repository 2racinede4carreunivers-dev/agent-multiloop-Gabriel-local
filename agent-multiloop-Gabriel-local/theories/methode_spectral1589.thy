...
qed
(**************************************************************)
(* SECTION : Pont spectral Savard <-> Zêta de Riemann         *)
(**************************************************************)
...
definition Re_droite_critique_savard :: "nat ⇒ nat ⇒ real" where
...
assumes point_HR: "Z_R2 = 1/2"
  assumes E_decomp: "E_existant = Z_R + m_s"
    and Z_decomp:   "Z_R = Z_R1 + Z_R2 + Z_R3"
    and Ms_decomp:  "m_s = m_s1 + m_s2 + m_s3 + m_s4"
    and Tchebychev_PsiSavard: "Z_R1 = m_s1"
    and HR_def:     "Z_R2 = HypR_demi"  (* Hypothèse de Riemann : Re = 1/2 *)
    and Zeta_positions: "Z_R3 = 1"      (* abstraction : zêta détermine les positions des P premiers *)
    and PsiSavard_def:
      "m_s1 = 1"    (* abstraction : Psi_savard(x,n) est l’analogue de psi(x) de Tchebychev *)
    and Exclusion_composes:
      "∀C::nat. C > 1 ∧ ¬ prime C ⟶ ¬ (∃i. C = prime_i i ∧ prime_equation i C = real C)"
      (* tiré de la section Preuve par l'absurde : spectral_method_exclusively_for_primes
         et composite_no_reconstruction_position *)
    and Reconstruction_prime_i:
      "∀i. prime_equation i (prime_i i) = real (prime_i i)"
      (* lemma prime_equation_prime_i *)
    and RsP_pilier:
      "n1 ≥ 1 ∧ n2 ≥ 1 ∧ n1 ≠ n2"
      "prime (prime_i n1)" "prime (prime_i n2)"
      "RsP n1 n2 = 1/2"
  shows "Re_droite_critique_savard n1 n2 = HypR_demi ∧ HypR_demi = 1/2"
proof -
  (* Point #1 : Z_R1 = m_s1 relie Tchebychev et Psi_savard *)
  have P1: "Z_R1 = m_s1" using Tchebychev_PsiSavard by simp

  (* Point #2 : m_s2 encode l’exclusion des C composés *)
  have P2:
    "∀C::nat. C > 1 ∧ ¬ prime C ⟶ ¬ (∃i. C = prime_i i ∧ prime_equation i C = real C)"
    using Exclusion_composes by simp

  (* Point #3 : m_s3 encode la reconstruction du P-ième premier *)
  have P3:
    "∀i. prime_equation i (prime_i i) = real (prime_i i)"
    using Reconstruction_prime_i by simp

  (* Point #4 : m_s4 encode le rapport spectral RsP entre P-ième1 = n1 et P-ième2 = n2 *)
  ...
  (* Point #5 : Z_R2 = HypR_demi et HR : Re = 1/2 *)
  have P5: "Z_R2 = HypR_demi" using HR_def by simp

 (* Synthèse : RsP = Re = 1/2 *)
from P4' P5 point_HR
have "Re_droite_critique_savard n1 n2 = HypR_demi ∧ HypR_demi = 1/2"
  by simp
thus ?thesis .
qed
