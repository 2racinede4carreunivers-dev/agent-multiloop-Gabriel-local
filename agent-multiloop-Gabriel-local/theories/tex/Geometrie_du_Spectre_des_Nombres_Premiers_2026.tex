...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\scriptsize\href{https://www.universestaucarre.com}{www.universestaucarre.com}}
\fancyfoot[C]{\thepage}
...
Co-auteurs & Auteur principal : Philippe Thomas Savard
 ...
 Copilot — Microsoft E1 — emergent.sh Gordon — Docker Desktop Claude API — Anthropic Gemini — Google Ces co-auteurs ont contribué à la formalisation Isabelle/HOL, à la rédaction assistée, à la vérification des preuves , chacun à part égale avec l'auteur principal. \\
\hline
...
https://github.com/2racinede4carreunivers-dev/Ia_geo_spec_prem_app_deplo.git \\
\hline
\end{longtable}
...
\begin{longtable}{|p{0.940\textwidth}|}
\hline
Note liminaire \\ .L’ensemble des informations mathématiques est tiré des documents sources et du fichier methode_spectrale.thy. Vous êtes chaleureusement invités à consulter les README des dépôts publics, à contribuer via PR, à partager ou cloner les projets conformément aux permissions de la LICENSE Apache 2.0. Bienvenue dans la communauté. Méthode Spectrale de Philippe Thomas Savard. \\
\hline
\end{longtable}

\begin{longtable}{|p{0.940\textwidth}|}
\hline
Note méthodologique de correction \\ La correction de ce document a été appliquée avec un pipeline programmatique reproductible. Correcteurs utilisés : (1) \textbf{LanguageTool} via script Python lorsque disponible (API publique puis moteur local), (2) \textbf{normalisation terminologique} via le fichier \texttt{termbase\_ca\_qc.csv}, (3) \textbf{garde sémantique} par score de préservation (seuil 9.6/10) avant acceptation d'une modification. Le pipeline utilisé est \texttt{theories/tex/tex\_quality/quality\_pipeline.py} et le rapport de traçabilité est produit en JSON dans \texttt{theories/tex/qa\_output/}. \\ 
\hline
\end{longtable}
...
\begin{longtable}{|p{0.940\textwidth}|}
\hline
P5 — Exclusivité sur ℙ \\ Localement à la Méthode Spectrale, l'image de \texttt{prime\_i} est exclusivement composée de nombres premiers. La preuve par l'absurde formalisée dans \texttt{methode\_spectral.thy} montre qu'un entier composé C ne peut pas être égal à \texttt{prime\_i(i)} pour un index i quelconque. En conséquence, la méthode caractérise exactement l'ensemble ℙ des nombres premiers, ni plus ni moins. \\
\hline
\end{longtable}


\begin{holblock}[title={Citation tiree du fichier methode\_spectral.thy lignes 1541 a 1554 : "Preuve par l'absurde" sur l'exclusion des composes pour le RsP}]
\begin{holcode}[title={Code HOL cite mot pour mot (indentation conservee)}]
...
Tchebychev $x^p/p$ -- exemple pour 29 :
$4^4/4 + 3^3/3 + 5^2/5 + 7^1/1 + 11^1/1 + 13^1/1 + 17^1/1 + 19^1/1 + 23^1/1 + 29^1/1 = 28.4765$
...
Les deux suites fondamentales du régime 1/2 sont définies par des formules fermées, calculables directement pour toute entier n ≥ 1 :

SA(n) = (3,25 / 2) × 2n − 2

SB(n) = (6,5 / 2) × 2n − 66

Il est crucial de comprendre d'où viennent les constantes 3,25 et 6,5. Elles ne sont pas choisies arbitrairement pour simplifier une fraction algébrique : elles émergent des sommes réelles des suites A et B construites par Philippe Thomas Savard. Ces suites portent, terme à terme, les valeurs associées aux positions n = 1, 2, 3, 4, 5, 6, ... correspondant aux premiers (2, 3, 5, 7, 11, 13, ...). Ce point constitue l'une des pierres angulaires de la méthode : la primauté du numérique réel sur l'algébrique formel.
...
Dans l'argument numérique avancé par Savard, lorsque n <= -1 (régime des premiers négatifs), le plus grand premier négatif est -2, puis viennent -3, -5, etc. En appliquant psi(Savard), on observe que dans l'expression $2^{-n}/SB_n$, la somme B tend rapidement vers une valeur quasi constante autour de -66 (selon la forme $3.25 \times 2^{-n} - 66$). Pour n <= -15, SB_n est pratiquement égal à -66. Cette stabilisation soutient l'idée que les rapports spectraux restent à 1/2, y compris lorsque l'approche devient algébrique pour traiter de très grandes valeurs positives.
...
\begin{holcode}[title={Code HOL — Régime positif (n ≥ 1)}]
...
\begin{holcode}[title={Code HOL — Régime négatif (n ≤ −1, k = 2)}]
...
\begin{holcode}[title={Code HOL — Définition de RsP négatif}]
...
\begin{holcode}[title={Code HOL — Théorème RsP = 1/2 (régime central)}]
...
\begin{holcode}[title={Code HOL — Contraste local/global}]
...
\begin{holcode}[title={Code HOL — Identité de reconstruction du premier}]
...
\begin{holcode}[title={Index HOL — Validation des cinq premiers exemples}]
...
lemmas digamma_checks =
	digamma_calc_23 digamma_calc_29 digamma_calc_31 digamma_calc_37 digamma_calc_41
\end{holcode}
...
\end{enumerate}
\begin{holcode}[title={Code HOL — Premier 947}]
...
A1/3(n) = ((73/9)/6) × 3n − 1,5

B1/3(n) = ((219/9)/6) × 3n − (487 × 1,5)
...
\begin{holcode}[title={Code HOL — Théorème RsP = 1/3}]
...
\textbf{Convention de lecture des blocs}\\
Bloc A négatif\\
Bloc B positif

\bigskip

\textbf{Ex. 1.} 1 terme négatif, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \frac{13}{8} - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 49{,}625 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} - \frac{13}{4} - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -24{,}75 \quad \text{Somme suite B} \\
&\text{Digamma}: 49{,}625 + \frac{2651}{64} = 91{,}046875 \\
&\frac{-24{,}75 - 91{,}046875}{\dfrac{1}{64}} = 4243 \quad \text{Nombre premier}
\end{align*}

\bigskip

\textbf{Ex. 2.} 2 termes négatifs, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \left(\frac{13}{8} + \frac{13}{16}\right) - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 48{,}8125 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} + \left(\frac{13}{4} + \frac{13}{8}\right) - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -26{,}375 \quad \text{Somme suite B} \\
&\text{Digamma}: 48{,}8125 - \frac{8727}{64} = -94{,}546875 \\
&\frac{-26{,}375 - (-94{,}546875)}{\dfrac{1}{64}} = 4363 \quad \text{Nombre premier}
\end{align*}

\bigskip

\textbf{Ex. 3.} 3 termes négatifs, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \left(\frac{13}{8} + \frac{13}{16} + \frac{13}{32}\right) - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 48{,}40625 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} - \left(\frac{13}{4} + \frac{13}{8} + \frac{13}{16}\right) - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -26{,}375 \quad \text{Somme suite B} \\
&\text{Digamma}: 48{,}40625 - \frac{9059}{64} = -93{,}140625 \\
&\frac{-26{,}375 - (-93{,}140625)}{\dfrac{1}{64}} = 4273
\end{align*}

\bigskip

\textbf{Ex. 4.} 4 termes négatifs, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \left(\frac{13}{8} + \frac{13}{16} + \frac{13}{32} + \frac{13}{64}\right) - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 48{,}203125 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} - \left(\frac{13}{4} + \frac{13}{8} + \frac{13}{16} + \frac{13}{32}\right) - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -27{,}59375 \quad \text{Somme suite B} \\
&\text{Digamma}: 48{,}203125 - \frac{9248}{64} \ (4397) \\
&\frac{-27{,}59375 - (-96{,}296875) \ (4397)}{\dfrac{1}{64}} = 4397
\end{align*}

\bigskip

\textbf{Ex. 5.} 5 termes négatifs, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \left(\frac{13}{8} + \frac{13}{16} + \frac{13}{32} + \frac{13}{64} + \frac{13}{128}\right) - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 48{,}1015625 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} - \left(\frac{13}{4} + \frac{13}{8} + \frac{13}{16} + \frac{13}{32} + \frac{13}{64}\right) - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -27{,}796875 \quad \text{Somme suite B} \\
&\text{Digamma}: 48{,}1015625 - 144{,}7890625 = -96{,}6875 \\
&\frac{-27{,}796875 - (-96{,}6875)}{\dfrac{1}{64}} = 4409
\end{align*}

\bigskip

\textbf{Ex. 6.} 6 termes négatifs, 5 termes positifs :
\begin{align*}
&\left(\frac{13}{4} - \left(\frac{13}{8} + \frac{13}{16} + \frac{13}{32} + \frac{13}{64} + \frac{13}{128} + \frac{13}{256}\right) - 2\right) + \left(2 + 4 + 8 + 12 + 24\right) = 48{,}05078125 \quad \text{Somme suite A} \\
&\left(\frac{13}{2} - \left(\frac{13}{4} + \frac{13}{8} + \frac{13}{16} + \frac{13}{32} + \frac{13}{64} + \frac{13}{128}\right) - 66\right) + \left(-59{,}5 + 6{,}5 + 13 + 26 + 52\right) = -27{,}8984375 \quad \text{Somme suite B} \\
&\text{Digamma}: 48{,}05078125 - \frac{37127}{256} = -96{,}9765625 \\
&\frac{-27{,}8984375 - (-96{,}9765625)}{\dfrac{1}{64}} = 4421
\end{align*}
...
\begin{holcode}[title={Code HOL — Invariance RsP en régime négatif}]
...
L'asymétrie chaotique est une configuration dégradée où les indices ne suivent pas l'ordre naturel, où |A| ≠ |B|, et où la configuration ordonnée n'est pas satisfaite. Cette situation peut survenir dans des constructions non canoniques.
...
\begin{holcode}[title={Code HOL — Validité des indices en asymétrie}]
...
\begin{holcode}[title={Code HOL — Théorème central d'exclusion des composés}]
...
Pour construire une suite spectrale de longueur n ≥ 8, le manuscrit de Philippe Thomas Savard établit quatre règles précises :
...
\begin{description}
  \item[\textbf{Zêta ($\zeta$)}] Dans le système de numération grecque,
  le Zêta vaut~7 bien qu'il occupe la 6\ieme{} position. Cela est dû à
  l'ancienne existence du Digamma, situé entre l'Epsilon et le Zêta.%
  \footnote{Wikipédia, l'encyclopédie libre. \textit{Zêta}.
  Disponible en ligne : \url{https://fr.wikipedia.org/wiki/Zêta}}
\end{description}
...
\[
\psi_{\mathrm{Savard}}(x,n)=x-\frac{2^{n}}{SB(n)}-\log(2\pi)-\frac{1}{2}\log(1-x^{-2})
\]

La substitution $\sum_{\rho}(x^{\rho}/\rho) \to 2^{n}/SB(n)$ est la clé du pont. Elle transforme une somme sur des zéros complexes non triviaux en un ratio entre une puissance géométrique et la somme partielle SB, deux objets entièrement arithmétiques, construits à partir des premiers réels. Cette substitution est justifiée numériquement par les validations ci-dessous.
...
\begin{holcode}[title={Index HOL — Faits mobilisés par la synthèse}]
(* References explicites citees dans ce document *)
F2: hypothese_critique
F4: RsP_universel_entier_naturel
F5: composite_not_prime_i composite_no_reconstruction_position composite_pair_no_rsp_positions
